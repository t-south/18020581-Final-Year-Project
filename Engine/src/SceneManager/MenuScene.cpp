...
void geProject::MenuScene::setPicking()
{
	...
	if (mouse->mouseButtonDown(GLFW_MOUSE_BUTTON_LEFT)) {
		...
		case 0:
			...
			break;
		case 1:
			eventSystem.publish(new GameLoadEvent(GameplayContext, 2, false));
			
			break;
		case 2:
			eventSystem.publish(new GameLoadEvent(GameplayContext, 3, true));
			break;
		case 3:
			eventSystem.publish(new GameLoadEvent(GameplayContext, -1 , true));
			break;
		default:
			break;
		}
